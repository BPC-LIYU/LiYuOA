...
class MongoHandler(logging.Handler):
    ...
    def _connect(self):
        ...
        if self.capped:
            try:  # We don't want to override the capped collection (and it throws an error anyway)
                self.collection = Collection(self.db, self.collection_name, capped=True, max=self.capped_max,
            ...
            self.collection = self.db[self.collection_name]
        self.count_connection = self.db['count_logs']
    ...
    def _save_count(self, record):
        ...
        if record.exc_info is not None:
            stackTrace = self.formatException(record.exc_info)
            md5_stackTrace = hashlib.md5(stackTrace).hexdigest()
            message = str(record.exc_info[1])
            docs = self.count_connection.find({"md5": md5_stackTrace})
            if docs.count() > 0:
                doc = docs[0]
                doc['count'] = doc.get('count', 1) + 1
                doc['last_time'] = datetime.datetime.now()
                self.count_connection.update({"md5": md5_stackTrace}, doc)
            else:
                now = datetime.datetime.now()
                doc = {"md5": md5_stackTrace, "stackTrace": stackTrace, "message": message, "count": 1,
                       "first_time": now, "last_time": now}
                self.count_connection.save(doc)
